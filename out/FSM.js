...
export class FSM {
    ...
    _finalize() {
        ...
        // bind start state transitions
        if (this._startState) {
            this._startState.transitions.forEach(transition => {
                transition.bindTarget(this._states);
                transition.onEvent.bindRegion(this._regions);
                transition.actions.forEach(action => action.bindRegion(this._regions));
            });
        }
        // **** YOUR CODE HERE ****
        // bind remaining state transitions
        ...
        this._regions.forEach(region => region.parent = this);
    }
    //. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
    ...
    actOnEvent(evtType, reg) {
        ...
        if (!this.currentState)
            return;
        // **** YOUR CODE HERE ****
        let match = this.currentState.transitions.find(transition => transition.match(evtType, reg));
        if (match) {
            match.actions.forEach(action => action.execute(evtType, reg));
            this._currentState = match.target;
        }
    }
    //-------------------------------------------------------------------
    // Debugging Support
