...
export class FSMInteractor {
    ...
    protected _previous: Array<string>;
    public get previous() {return this._previous}
    public set previous(v: Array<string>) {
        this._previous = v;
    }
    ...
    public pick(localX : number, localY : number) : Region[] {
        ...
        // **** YOUR CODE HERE ****
        // filter regions and reverse order
        pickList = this.fsm.regions
            .filter(region => region.pick(localX, localY))
            .reverse();
                            ...
                            localX : number, localY : number)
    {
        ...
        // **** YOUR CODE HERE ****
        const picked = this.pick(localX, localY);
        const event: EventSpec[] = [];

        // handle exit
        this.previous.forEach(regionName => {
            if (!picked.some(r => r.name === regionName)) {
                event.push(new EventSpec('exit', regionName));
            }
        });

        // handle enter and move inside
        picked.forEach(region => {
            if (!this.previous.includes(region.name)) {
                event.push(new EventSpec('enter', region.name));
            } else if (what === 'move') {
                event.push(new EventSpec('move_inside', region.name));
            }
        });

        // handle press and release
        if (what === 'press' || what === 'release') {
            picked.forEach(region => {
                event.push(new EventSpec(what, region.name));
            });
        }

        // update array storing previous states
        this.previous = picked.map(r => r.name);

        // dispatch events
        event.forEach(event => {
            if (this.fsm?.regions) {
                event.bindRegion(this.fsm.regions);
